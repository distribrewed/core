...
class BaseMaster:
    ...
    def ping_worker(self, worker_id):
        ...
        core.tasks.worker.handle_ping.apply_async(worker_id=worker_id)

    def handle_ping(self, worker_id):
        log.info("Received ping from '{0}'".format(worker_id))
        core.tasks.worker.handle_pong.apply_async(worker_id=worker_id)
    ...
    def command_all_workers_to_ping_master(self):
        ...
        core.tasks.worker.ping_master.apply_async(all_workers=True)

    def request_worker_method_list(self, worker_id):
        core.tasks.worker.send_method_list.apply_async(worker_id=worker_id)
    ...
    def call_worker_method_by_name(self, worker_id, method):
        core.tasks.worker.call_method_by_name.apply_async(worker_id=worker_id, args=[method])
